...
class CandidateSet(object):
            ...
            if self.multiprocessing > 1 and num_targets > 5:
                ...
                with ProcessPoolExecutor(max_workers=workers) as ex:
                    try:
                        ...
                        futures = {ex.submit(multi_target_data, ticid_group, self.data.loc[ticid_group], self.lc_dir, self.dir_style, skip_per_correction): ticid_group for ticid_group in ticid_split}
                ...
                for ticid_group in ticid_split:
                    filled, fail = multi_target_data(ticid_group, self.data.loc[ticid_group], self.lc_dir, self.dir_style, skip_per_correction)
            ...
            if self.multiprocessing > 1 and num_targets > 5:
                ...
                with ProcessPoolExecutor(max_workers=workers) as ex:
                    try:
                        ...
                        futures = {ex.submit(multi_centroid, self.sector_data.loc[index_group], 
                                             self.lc_dir, self.dir_style, self.plot_centroid): index_group for index_group in index_split}
                ...
                # Run on a single core
                filled, fails = multi_centroid(self.sector_data.loc[to_fill.index], self.lc_dir, self.dir_style, self.plot_centroid)
    ...
    def generate_features(self, infile_features=None, infile_som=None, rerun=False, mask_multi = False):
        ...
        Will have to be done/updated for each potential target star, to account for transit depth
        """
        ...
        if self.multiprocessing > 1 and num_targets > 5:
            ...
            while num_targets/(factor*workers) < 8:
                ...
                if factor == 1:
                    break
            ticid_split = np.array_split(candidates_to_run.unique('ticid').values, factor*workers)
            
            with ProcessPoolExecutor(max_workers=workers) as ex:
                futures = {ex.submit(multi_features, 
                                     list(map(self.sources.get, ticid_group)),
                                     self.data.loc[ticid_group],
                                     self.sector_data.loc[ticid_group],
                                     self.lc_dir, self.dir_style, mask_multi):ticid_group for ticid_group in ticid_split}
            ...
            for target_id in candidates_to_run.unique('ticid'):
                try:
                    features_df, som_arrays = single_target_features(target_id, 
                                                                     self.sources[target_id], 
                                                                     self.data.loc[target_id], 
                                                                     self.sector_data.loc[target_id], 
                                                                     self.lc_dir, self.dir_style, mask_multi)
                    df_lst.append(features_df)
                    som_array_lst.extend(som_arrays)
                except Exception as e:
...
# Define a lot of statitic methods that will be used by the class
def multi_target_data(ticids, group_data, lc_dir, dir_style, skip_per_correction):
    """
    ...
    for ticid in ticids:
        try:
            ...
            results.append(per_sector_data(ticid, group_data.loc[ticid], lc_dir, dir_style, skip_per_correction, functions))
        except Exception as e:
...
def per_sector_data(ticid, initial_data, lc_dir, dir_style, skip_per_correction, functions=None):
    """
    ...
    sec_error = []
    for sec in sectors:
        lcfile = cutils.lc_filepath(lc_dir, dir_style, ticid, sec)
...
def multi_centroid(group_sec_data, lc_dir, dir_style, plot_centroid):
    """
    ...
    with warnings.catch_warnings():
        ...
        for idx in group_sec_data.index:
            ticid, cndt, sec = idx
            try:
                # Store the results in a list, to be used for the construction of a dataframe
                results.append(observed_centroid_offset(group_sec_data.loc[idx], lc_dir, dir_style, plot_centroid))
            except Exception as e:
                # Store the failure cases and their associated exception
                fails.append((idx, e))
...
def observed_centroid_offset(event_data, lc_dir, dir_style, plot_centroid):
    """
    ...
    ticid, cndt, sec = event_data.name
    ...
    lc_file = cutils.lc_filepath(lc_dir, dir_style, ticid, sec)
...
def multi_features(sources, group_data, group_sector_data, lc_dir, dir_style, mask_multi):
    feat_lst = []
    failed = []
    som_array_lst = []
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        for i, ticid in enumerate(group_data.index.unique('ticid')):
            try:
                features_df, som_arrays = single_target_features(ticid, sources[i], 
                                                                group_data.loc[ticid], 
                                                                group_sector_data.loc[ticid],
                                                                lc_dir, dir_style, mask_multi)
...
def single_target_features(target_id, source, target_data, target_sector_data, lc_dir, dir_style, mask_multi=False):
    tce_feat_lst = []
    som_array_lst = []
    ...
    target_sector_data = target_sector_data.copy()
    ...
    lcs = {}
    for sec in sectors:
        lc_file = cutils.lc_filepath(lc_dir, dir_style, target_id, sec)
        try:
    ...
    for tce in target_data.index:
        ...
        if mask_multi and len(target_data.index) > 1:
